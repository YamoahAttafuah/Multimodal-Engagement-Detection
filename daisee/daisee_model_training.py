...
from sklearn.metrics import precision_recall_curve, classification_report, multilabel_confusion_matrix
...
SELECTED_FEATURES_PATH = 'selected_features_53.json'
MODEL_PATH = 'rf_daisee_model_GAMMA_53features.h5'   # For saving the model
...
def create_subject_independent_splits(all_labels_df):
    ...
    print(f"Number of unique subjects found: {unique_subjects}")
...
def build_daisee_model(input_shape=(75, 53)):
...
def evaluate_performance(model, test_gen, test_df, history):
    ...
    if history:
        ...
        plt.plot(epochs_range, metrics['val_binary_accuracy'], label='Validation Accuracy', linestyle='--', linewidth=2)
    ...
    for i, (name, matrix) in enumerate(zip(class_names, mcm)):
        plt.subplot(2, 2, i + 1)
        sns.heatmap(matrix, annot=True, fmt='d', cmap='Blues', cbar=False,
                    xticklabels=['Pred Low', 'Pred High'],
                    yticklabels=['True Low', 'True High'],
                    annot_kws={"size": 14})
        plt.title(f"{name} (Threshold: {best_thresholds[name]:.2f})", fontsize=16)
